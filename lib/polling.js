		...
		for (var i = 0; i < sent.length; i++) {
			...
			if (!offers[offer.id]) {
				...
				if (!offer.isGlitched()) {
					...
					var countWithoutName = !this._language ? 0 : offer.itemsToGive.concat(offer.itemsToReceive).filter(function (item) {
						return !item.name;
					}).length;
