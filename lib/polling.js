...
TradeOfferManager.prototype.doPoll = function(doFullUpdate) {
	...
	this.getOffers(fullUpdate ? EOfferFilter.All : EOfferFilter.ActiveOnly, new Date(offersSince * 1000), (err, sent, received) => {
		...
		let origPollData = JSON.parse(JSON.stringify(this.pollData)); // deep clone
		...
		/*if (fullUpdate) {
			...
			let trackedIds = sent.map(offerId).concat(received.map(offerId)); // OfferIDs that are active in Steam's memory
			let oldIds = Object.keys(this.pollData.sent || {}).concat(Object.keys(this.pollData.received || {})); // OfferIDs that we have in our poll data
			...
			oldIds.forEach((offerID) => {
				if (trackedIds.indexOf(offerID) == -1) {
					...
					let offerAgeDays = this.pollData.timestamps && this.pollData.timestamps[offerID] ? (Date.now() - this.pollData.timestamps[offerID]) / (1000 * 60 * 60 * 24) : null;
			...
			let knownTimestamps = Object.keys(this.pollData.timestamps || {});
			knownTimestamps.forEach((offerID) => {
				let isSent = this.pollData.sent && this.pollData.sent[offerID];
				let isReceived = this.pollData.received && this.pollData.received[offerID];
		...
		let timestamps = this.pollData.timestamps || {};
		...
		sent.forEach((offer) => {
			if (!offers[offer.id]) {
				...
				if (!offer.isGlitched()) {
					...
					let countWithoutName = !this._language ? 0 : offer.itemsToGive.concat(offer.itemsToReceive).filter(function(item) { return !item.name; }).length;
			...
			if (offer.state == ETradeOfferState.Active) {
				...
				let customCancelTime = offer.data('cancelTime');
			...
			if (offer.state == ETradeOfferState.CreatedNeedsConfirmation && this.pendingCancelTime) {
				...
				let customPendingCancelTime = offer.data('pendingCancelTime');
		...
		if (this.cancelOfferCount) {
			let sentActive = sent.filter(offer => offer.state == ETradeOfferState.Active);
		...
		if (!hasGlitchedOffer) {
			...
			sent.concat(received).forEach((offer) => {
				let updated = Math.floor(offer.updated.getTime() / 1000);
				if (updated > latest) {
					latest = updated;
				}
			});
		...
		if (!StdLib.Objects.deepEqual(origPollData, this.pollData)) {
