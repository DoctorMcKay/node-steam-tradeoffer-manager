...
TradeOfferManager.prototype.doPoll = function (doFullUpdate) {
	...
	this.emit(
		"debug",
		`Doing trade offer poll since ${offersSince}${fullUpdate ? " (full update)" : ""}`,
	);
	...
	this.getOffers(
		...
		(err, sent, received) => {
			...
			this.emit(
				"debug",
				`Trade offer poll succeeded in ${Date.now() - requestStart} ms`,
			);

			const origPollData = JSON.parse(JSON.stringify(this.pollData)); // deep clone
			...
			sent.forEach((offer) => {
				if (!offers[offer.id]) {
					...
					if (!offer.isGlitched()) {
						...
						this.emit(
							"debug",
							`Not emitting sentOfferChanged for ${offer.id} right now because it's glitched (${offer.itemsToGive.length} to give, ${offer.itemsToReceive.length} to receive, ${countWithoutName} without name)`,
						);
					}
				}
					...
					if (
						...
						offer.cancel((err) => {
							if (!err) {
								...
								this.emit(
									"debug",
									`Can't auto-canceling pending-confirmation offer #${offer.id}: ${err.message}`,
								);
							}
						});
					}
