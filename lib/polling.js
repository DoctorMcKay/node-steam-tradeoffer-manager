	...
	this.getOffers(fullUpdate ? EOfferFilter.All : EOfferFilter.ActiveOnly, new Date(offersSince * 1000), (err, sent, received) => {
		...
		if (fullUpdate) {
			...
			oldIds.forEach((offerID) => {
				if (trackedIds.indexOf(offerID) == -1) {
					...
					if (this.pollData.sent && this.pollData.sent[offerID] && this.pollData.sent[offerID] != ETradeOfferState.Active) {
						found = found || delete this.pollData.sent[offerID];
					}

					if (this.pollData.received && this.pollData.received[offerID] && this.pollData.sent[offerID] != ETradeOfferState.Active) {
		...
		sent.forEach((offer) => {
			if (!offers[offer.id]) {
				...
				// Only emit the unknownOfferSent event if currently theres no offers that await a response in .send
