 ...
 *  - `sent` is an object whose keys are offer IDs for known offers we've sent
             and whose values are the last known states of those offers
 ...
 *  	- `cancelTime` - The time, in milliseconds, after which the offer should be canceled automatically.
              Defaults to the TradeOfferManager's set cancelTime.
...
TradeOfferManager.prototype.doPoll = function (doFullUpdate) {
	...
	if (!this.apiKey || Date.now() - this._lastPoll < 1000) {
		// Either we don't have an API key, or we last polled
		// less than a second ago... we shouldn't spam the API.
	...
	var filter = fullUpdate ? EOfferFilter.All : EOfferFilter.ActiveOnly;
	this.getOffers(filter, new Date(offersSince * 1000), (getOffersErr, sent, received) => {
		if (getOffersErr) {
			this.emit('debug', 'Error getting trade offers for poll: ' + getOffersErr.message);
			this.emit('pollFailure', getOffersErr);
		...
		/* if (fullUpdate) {
			// We can only purge stuff if this is a full update; otherwise,
			// lack of an offer's presence doesn't mean Steam forgot about it.

			// OfferIDs that are active in Steam's memory
			var trackedIds = sent.map(offerId).concat(received.map(offerId));

			// OfferIDs that we have in our poll data
			var oldIds = Object.keys(this.pollData.sent || {}).concat(Object.keys(this.pollData.received || {}));

			// This routine won't delete any offers that we last saw as Active.
			// If it changed state and we didn't see it, it'll stick around forever.
			// Perhaps someday we should account for this as well; e.g. by clearing offers
			...
			oldIds.forEach((offerID) => {
				if (trackedIds.indexOf(offerID) == -1) {
					...
					var offerAgeDays = this.pollData.timestamps && this.pollData.timestamps[offerID]
						? (Date.now() - this.pollData.timestamps[offerID]) / (1000 * 60 * 60 * 24)
						: null;
					...
					if (this.pollData.sent
						&& this.pollData.sent[offerID]
						&& (this.pollData.sent[offerID] != ETradeOfferState.Active
						|| (offerAgeDays && offerAgeDays > 30))) {
						found = found || delete this.pollData.sent[offerID];
					}

					if (this.pollData.received
						&& this.pollData.received[offerID]
						&& (this.pollData.received[offerID] != ETradeOfferState.Active
						|| (offerAgeDays && offerAgeDays > 30))) {
			...
			knownTimestamps.forEach((offerID) => {
				...
				if (!isSent
					&& !isReceived
					&& Date.now() - this.pollData.timestamps[offerID] >= (1000 * 60 * 60 * 24 * 60)) {
					this.emit('debug','Cleaning up stale timestamp '
						+ this.pollData.timestamps[offerID] + ' for offer ' + offerID + ' from poll data');
		...
		sent.forEach((offer) => {
			if (!offers[offer.id]) {
				...
				// Apparently offers can appear in the API before the send() call has returned,
				// so we'll need to add a delay. Only emit the unknownOfferSent event
				// if currently there's no offers that await a response in .send
				...
				if (!offer.isGlitched()) {
					...
					var countWithoutName = !this._language ? 0 : offer.itemsToGive
						.concat(offer.itemsToReceive)
						.filter(item => !item.name).length;

					this.emit('debug', 'Not emitting sentOfferChanged for ' + offer.id
			...
			if (offer.state === ETradeOfferState.Active) {
				...
				if (cancelTime && (Date.now() - offer.updated.getTime() >= cancelTime)) {
					offer.cancel((offerCancelErr) => {
						if (!offerCancelErr) {
							this.emit('sentOfferCanceled', offer, 'cancelTime');
						} else {
							this.emit('debug', 'Can\'t auto-cancel offer #' + offer.id + ': ' + offerCancelErr.message);
						}
					});
				}
			}
			...
			if (offer.state === ETradeOfferState.CreatedNeedsConfirmation && this.pendingCancelTime) {
				// The offer needs to be confirmed to be sent.
				// Let's see if the maximum time has elapsed before we cancel it.
				...
				if (pendingCancelTime && (Date.now() - offer.created.getTime() >= pendingCancelTime)) {
					offer.cancel((err) => {
						if (!err) {
							...
							this.emit('debug', 'Can\'t auto-canceling pending-confirmation offer #'
								+ offer.id + ': ' + err.message);
						}
					});
				}
			}
		...
		received.forEach((offer) => {
			...
			if (offer.fromRealTimeTrade) {
				// This is a real-time trade offer
				if (!offers[offer.id] && offer.needsConfirmation()) {
					this.emit('realTimeTradeConfirmationRequired', offer);
				} else if (offer.state === ETradeOfferState.Accepted
