...
TradeOffer.prototype.send = function(callback) {
	...
	if (this.itemsToGive.length + this.itemsToReceive.length) {
...
TradeOffer.prototype.getExchangeDetails = function(getDetailsIfFailed, callback) {
	...
	function makeRequest(tryOneDayEscrow) {
		...
		if (hadEscrow && offer.state != ETradeOfferState.InEscrow) {
			...
			timestamp = Math.floor(offer.escrowEnds.getTime() / 1000) - (60 * 60 * 24 * (tryOneDayEscrow ? 1 : 15));
		}
		...
		offer.manager._apiCall("GET", "GetTradeHistory", 1, {"max_trades": 100, "include_failed": 1, "start_after_time": timestamp + 60}, (err, result) => {
			...
			if (!trade && hadEscrow && offer.state != ETradeOfferState.InEscrow && !tryOneDayEscrow) {
				// Maybe this trade was held for 24 hours?
				makeRequest(true);
				return;
			} else if (!trade) {
