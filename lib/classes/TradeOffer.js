...
TradeOffer.prototype.isGlitched = function() {
	...
	if (this.itemsToGive.concat(this.itemsToReceive).some(function(item) { return !item.name; })) {
		return true;
	}
...
function addItem(details, offer, list) {
	...
	if(list.some(function(tradeItem) { return Helpers.itemEquals(tradeItem, item); })) {
		// Already in trade
		return false;
	}
