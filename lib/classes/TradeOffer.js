...
TradeOfferManager.prototype.getOffers = function(filter, historicalCutoff, callback) {
	...
	this._apiCall('GET', 'GetTradeOffers', 1, options, function(err, body) {
		...
		// Make sure all the offers are well-formed
		if((body.response.trade_offers_sent || []).concat(body.response.trade_offers_received || []).some(offerMalformed)) {
			callback(new Error("Data temporarily unavailable"));
			return;
		}
...
function offerMalformed(offer) {
	return !offer.accountid_other || ((offer.items_to_give || []).length == 0 && (offer.items_to_receive || []).length == 0);
}
