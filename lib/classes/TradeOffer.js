	...
	switch (key) {
		case 'cancelTime':
			...
			if (this.id && this.state !== ETradeOfferState.Active) {
				throw new Error(`Cannot set cancelTime for offer #${this.id} \
as it is not active (${ETradeOfferState[this.state]}).`);
			}
...
function addItem(details, offer, list) {
	...
	if (typeof details.appid === 'undefined'
...
TradeOffer.prototype.cancel = TradeOffer.prototype.decline = function (callback) {
	...
	if (this.state !== ETradeOfferState.Active && this.state !== ETradeOfferState.CreatedNeedsConfirmation) {
		Helpers.makeAnError(new Error(`Offer #${this.id} is not active, \
so it may not be cancelled or declined`), callback);
		return;
	}

	this.manager._apiCall('POST',
		this.isOurOffer ? 'CancelTradeOffer' : 'DeclineTradeOffer', 1,
...
TradeOffer.prototype.accept = function (skipStateUpdate, callback) {
	...
	this.manager._community.httpRequestPost(`https://steamcommunity.com/tradeoffer/${this.id}/accept`, {
		...
		checkJsonError: false,
		checkHttpError: false, // we'll check it ourself. Some trade offer errors return HTTP 500
	}, (acceptOfferErr, response, body) => {
		if (acceptOfferErr || response.statusCode !== 200) {
			if (response && response.statusCode === 403) {
				...
				Helpers.makeAnError(acceptOfferErr || new Error('HTTP error ' + response.statusCode), callback, body);
			}
		...
		this.update((updateOfferErr) => {
			if (updateOfferErr) {
				callback(new Error('Cannot load new trade data: ' + updateOfferErr.message));
				return;
			}
...
TradeOffer.prototype.update = function (callback) {
	this.manager.getOffer(this.id, (err, offer) => {
		...
		// Clone only the properties that might be out of date from the new TradeOffer
		// onto this one, unless this one is glitched. Sometimes Steam is bad
		// and some properties are missing/malformed.
		var properties = [
			'id',
			'state',
			'expires',
...
TradeOffer.prototype.getReceivedItems = function (getActions, callback) {
	...
	this.manager._community.httpRequestGet(`https://steamcommunity.com/trade/${this.tradeID}/receipt/`,
		(receiptErr, response, body) => {
			if (receiptErr || response.statusCode !== 200) {
				Helpers.makeAnError(receiptErr || new Error('HTTP error ' + response.statusCode), callback);
				return;
			}

			// I believe this is now redundant thanks to httpRequestGet
			...
			if (!getActions) {
				...
				this.manager._addDescriptions(items, (descriptionsErr, describedItems) => {
					if (descriptionsErr) {
...
TradeOffer.prototype.getUserDetails = function (callback) {
	...
	this.manager._community.httpRequestGet(url, (err, response, body) => {
		...
		// Avatars
		var myAvatar = body.match(new RegExp('<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="'
			+ this.manager.steamID.accountid + '">'));
		var theirAvatar = body.match(new RegExp('<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="'
...
// A helper to use inside lib/polling.js
TradeOffer.prototype.needsConfirmation = function () {
	return this.state === ETradeOfferState.CreatedNeedsConfirmation
		|| (this.state === ETradeOfferState.Active
			&& this.confirmationMethod !== EConfirmationMethod.None);
};
