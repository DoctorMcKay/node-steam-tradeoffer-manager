...
function createOfferFromData(manager, data) {
	...
	offer.itemsToGive = processItems(data.items_to_give || []);
	offer.itemsToReceive = processItems(data.items_to_receive || []);
...
function processItems(items) {
	return items.map(function(item) {
		...
		item.classid = item.classid.toString();
		item.instanceid = item.instanceid.toString();
	...
	this.manager._request({
		...
		if(!body.success) {
			if(body.Error) {
				...
				callback(new Error("Unsuccessful request"));
			}
		...
		this.manager._digestDescriptions(body.rgDescriptions);
		
		data = (data || []).concat(this.manager._mapItemsToDescriptions(appid, contextid, body.rgInventory)).concat(this.manager._mapItemsToDescriptions(appid, contextid, body.rgCurrency));
		if(body.more) {
