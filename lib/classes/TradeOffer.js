 ...
 * @returns {Promise<{inventory: Object[], currencies: Object[]}>}
 */
TradeOffer.prototype.getPartnerInventoryContents = function(appid, contextid, callback) {
	return this.manager.getUserInventoryContents(this.partner, appid, contextid, true, callback);
};

/**
 * Add one of your items to this trade offer.
 * @param {{appid: int, contextid: int|string, assetid?: int|string, id?: int|string}} item
 ...
 * @param {{appid: int, contextid: int|string, assetid?: int|string, id?: int|string}[]} items
...
TradeOffer.prototype.send = function(callback) {
	return StdLib.Promises.callbackPromise(['status'], callback, true, (resolve, reject) => {
		if (this.id) {
			Helpers.makeAnError(new Error('This offer has already been sent'), reject);
			return;
		}

		if (this.itemsToGive.length + this.itemsToReceive.length == 0) {
			Helpers.makeAnError(new Error('Cannot send an empty trade offer'), reject);
			return;
		}
		...
		this.manager._community.httpRequestPost('https://steamcommunity.com/tradeoffer/new/send', {
			...
			if (err) {
				Helpers.makeAnError(err, reject);
				return;
			}
			...
			if (response.statusCode != 200) {
				if (response.statusCode == 401) {
					...
					Helpers.makeAnError(new Error('Not Logged In'), reject);
					return;
				}

				Helpers.makeAnError(new Error('HTTP error ' + response.statusCode), reject, body);
				return;
			}

			if (!body) {
				Helpers.makeAnError(new Error('Malformed JSON response'), reject);
				return;
			}

			if (body && body.strError) {
				Helpers.makeAnError(null, reject, body);
				return;
			}
...
TradeOffer.prototype.cancel = TradeOffer.prototype.decline = function(callback) {
	return StdLib.Promises.callbackPromise([], callback, true, async (resolve, reject) => {
		if (!this.id) {
			Helpers.makeAnError(new Error('Cannot cancel or decline an unsent offer'), reject);
			return;
		}

		if (this.state != ETradeOfferState.Active && this.state != ETradeOfferState.CreatedNeedsConfirmation) {
			Helpers.makeAnError(new Error(`Offer #${this.id} is not active, so it may not be cancelled or declined`), reject);
			return;
		}

		try {
			await this.manager._apiCall('POST', this.isOurOffer ? 'CancelTradeOffer' : 'DeclineTradeOffer', 1, {tradeofferid: this.id});
			this.state = this.isOurOffer ? ETradeOfferState.Canceled : ETradeOfferState.Declined;
			this.updated = new Date();

			this.manager.doPoll();
		} catch (ex) {
			Helpers.makeAnError(ex, reject);
		}
	});
};
...
TradeOffer.prototype.accept = function(skipStateUpdate, callback) {
	...
	return StdLib.Promises.callbackPromise(['status'], callback, true, (resolve, reject) => {
		if (!this.id) {
			Helpers.makeAnError(new Error('Cannot accept an unsent offer'), reject);
			return;
		}

		if (this.state != ETradeOfferState.Active) {
			Helpers.makeAnError(new Error(`Offer #${this.id} is not active, so it may not be accepted`), reject);
			return;
		}

		if (this.isOurOffer) {
			Helpers.makeAnError(new Error(`Cannot accept our own offer #${this.id}`), reject);
			return;
		}
		...
		this.manager._community.httpRequestPost(`https://steamcommunity.com/tradeoffer/${this.id}/accept`, {
			...
			if (err || response.statusCode != 200) {
				if (response && response.statusCode == 403) {
					...
					Helpers.makeAnError(new Error('Not Logged In'), reject, body);
				} else {
					Helpers.makeAnError(err || new Error('HTTP error ' + response.statusCode), reject, body);
				}
			...
			if (skipStateUpdate) {
				...
				return resolve({status: 'accepted'});
			}

			try {
				await this.update();
			} catch (ex) {
				return reject('Cannot load new trade data: ' + ex.message);
			}
			...
			if (this.state == ETradeOfferState.InEscrow) {
				return resolve({status: 'escrow'});
			}
			...
			return reject(new Error('Unknown state ' + this.state));
		}, 'tradeoffermanager');
	});
};
...
TradeOffer.prototype.update = function(callback) {
	return StdLib.Promises.callbackPromise([], callback, false, async (resolve, reject) => {
		let offer = await this.manager.getOffer(this.id);
 ...
 * @param {function} [callback]
 * @returns {Promise<{tradeStatus: int, tradeInitTime: Date, itemsReceived: Object[], itemsGiven: Object[]}>}
 */
TradeOffer.prototype.getExchangeDetails = function(getDetailsIfFailed, callback) {
	...
	let args = ['tradeStatus', 'tradeInitTime', 'itemsReceived', 'itemsGive'];
	return StdLib.Promises.callbackPromise(args, callback, false, async (resolve, reject) => {
		if (!this.id) {
			Helpers.makeAnError(new Error('Cannot get trade details for an unsent trade offer'), reject);
			return;
		}

		if (!this.tradeID) {
			Helpers.makeAnError(new Error('No trade ID; unable to get trade details'), reject);
			return;
		}

		let result;
		try {
			result = await this.manager._apiCall('GET', 'GetTradeStatus', 1, {tradeid: this.tradeID});
		} catch (ex) {
			Helpers.makeAnError(ex, reject);
			return;
		}
		...
		if (!trade || trade.tradeid != this.tradeID) {
			Helpers.makeAnError(new Error('Trade not found in GetTradeStatus response; try again later'), reject);
			return;
		}

		if (!getDetailsIfFailed && ![ETradeStatus.Complete, ETradeStatus.InEscrow, ETradeStatus.EscrowRollback].includes(trade.status)) {
			Helpers.makeAnError(new Error('Trade status is ' + (ETradeStatus[trade.status] || trade.status)), reject);
			return;
		}

		let received = trade.assets_received || [];
		let given = trade.assets_given || [];

		if (this.manager._language) {
			// Map descriptions
			await this.manager._requestDescriptions(received.concat(given));
			received = this.manager._mapItemsToDescriptions(null, null, received);
			given = this.manager._mapItemsToDescriptions(null, null, given);
		}
 ...
 * @param {function} [callback]
 * @returns {Promise<{me: {personaName: string, contexts: Object, escrowDays: int, avatarIcon: string, avatarMedium: string, avatarFull: string}, them: {personaName: string, contexts: Object, escrowDays: int, probation: boolean, avatarIcon: string, avatarMedium: string, avatarFull: string}}>}
 */
TradeOffer.prototype.getUserDetails = function(callback) {
	return StdLib.Promises.callbackPromise(['me', 'them'], callback, false, async (resolve, reject) => {
		if (this.id && this.isOurOffer) {
			Helpers.makeAnError(new Error('Cannot get user details for an offer that we sent.'), reject);
			return;
		}

		if (this.id && this.state != ETradeOfferState.Active) {
			Helpers.makeAnError(new Error('Cannot get user details for an offer that is sent and not Active.'), reject);
			return;
		}
		...
		let {me, them} = await Helpers.getUserDetailsFromTradeWindow(this.manager, url);
		resolve({me, them});
	});
};
