...
/**
 * Get the items you received in this trade. You probably want to use getExchangeDetails instead.
 * @deprecated Use getExchangeDetails instead
 * @param {boolean} [getActions=false]
 * @param {function} callback
 */
TradeOffer.prototype.getReceivedItems = function(getActions, callback) {
	...
	this.manager._community.httpRequestGet(`https://steamcommunity.com/trade/${this.tradeID}/receipt/`, (err, response, body) => {
		...
		let match = body.match(/<div id="error_msg">\s*([^<]+)\s*<\/div>/); // I believe this is now redundant thanks to httpRequestGet
		if (match) {
			Helpers.makeAnError(new Error(match[1].trim()), callback);
			return;
		}

		let script = body.match(/(let oItem;[\s\S]*)<\/script>/);
		if (!script) {
...
TradeOffer.prototype.getExchangeDetails = function(getDetailsIfFailed, callback) {
	...
	this.manager._apiCall("GET", "GetTradeStatus", 1, {"tradeid": this.tradeID}, (err, result) => {
		if (err) {
			Helpers.makeAnError(err, callback);
			return;
		}
		...
		if (!this.manager._language) {
			...
			this.manager._requestDescriptions((trade.assets_received || []).concat(trade.assets_given || []), (err) => {
				...
				output.receivedItems = this.manager._mapItemsToDescriptions(null, null, trade.assets_received || []);
				output.sentItems = this.manager._mapItemsToDescriptions(null, null, trade.assets_given || []);
				callback(null, output);
			});
		}
	});
