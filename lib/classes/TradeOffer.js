...
TradeOffer.prototype.data = function (key, value) {
	...
	if (arguments.length < 1) {
		...
		return (pollData.offerData?.[this.id]) || {};
 ...
 */
TradeOffer.prototype.loadPartnerInventory = function (
	appid,
	contextid,
	callback,
) {
	this.manager.loadUserInventory(
		this.partner,
		appid,
		contextid,
		true,
 ...
 * @param {function} callback
 */
TradeOffer.prototype.getPartnerInventoryContents = function (
	appid,
	contextid,
	callback,
) {
...
function addItem(details, offer, list) {
	...
	const item = {
		...
		appid: Number.parseInt(details.appid, 10), // always needs to be an int
		contextid: details.contextid.toString(), // always needs to be a string
		amount: Number.parseInt(details.amount || 1, 10), // always needs to be an int
	};
...
TradeOffer.prototype.send = function (callback) {
	...
	this.manager._community.httpRequestPost(
		"https://steamcommunity.com/tradeoffer/new/send",
		{
			headers: {
				referer:
					`https://steamcommunity.com/tradeoffer/${this.id || "new"}/?partner=${
		...
		(err, response, body) => {
			...
			if (body?.tradeofferid) {
				...
				for (const i in this._tempData) {
					if (Object.hasOwn(this._tempData, i)) {
...
TradeOffer.prototype.cancel = TradeOffer.prototype.decline = function (
	callback,
) {
	if (!this.id) {
		Helpers.makeAnError(
	...
	this.manager._community.httpRequestPost(
		`https://steamcommunity.com/tradeoffer/${this.id}/${
			this.isOurOffer ? "cancel" : "decline"
		}`,
		{
			headers: {
				referer:
					`https://steamcommunity.com/tradeoffer/${this.id}/?partner=${this.partner.accountid}${this._token ? `&token=${this._token}` : ""}`,
			},
			json: true,
			form: {
				sessionid: this.manager._community.getSessionID(),
...
TradeOffer.prototype.getReceivedItems = function (getActions, callback) {
	...
	if (!this.tradeID) {
		Helpers.makeAnError(
			new Error(`Offer #${this.id} is accepted, but does not have a trade ID`),
			callback,
		);
		return;
	}
	...
	this.manager._community.httpRequestGet(
		...
		(err, response, body) => {
			...
			const match = body.match(/<div id="error_msg">\s*([^<]+)\s*<\/div>/); // I believe this is now redundant thanks to httpRequestGet
			if (match) {
			...
			const items = [];
			require("node:vm").runInNewContext(script[1], {
				UserYou: null,
				BuildHover: (str, item) => {
					items.push(item);
				},
...
TradeOffer.prototype.getExchangeDetails = function (
	getDetailsIfFailed,
	callback,
) {
	...
	this.manager._apiCall(
		...
		(err, result) => {
			...
			if (!this.manager._language) {
				...
				this.manager._requestDescriptions(
					...
					(err) => {
						...
						const received = this.manager._mapItemsToDescriptions(
							null,
							null,
							trade.assets_received || [],
						);
						const given = this.manager._mapItemsToDescriptions(
							null,
							null,
							trade.assets_given || [],
						);
...
TradeOffer.prototype.getUserDetails = function (callback) {
	...
	this.manager._community.httpRequestGet(url, (err, response, body) => {
		...
		const vmContext = require("node:vm").createContext({
			UserYou: {
				SetProfileURL: () => {},
				SetSteamId: () => {},
			},
		...
		require("node:vm").runInContext(script, vmContext);
		...
		if (myEscrow && theirEscrow) {
			me.escrowDays = Number.parseInt(myEscrow[1], 10);
			them.escrowDays = Number.parseInt(theirEscrow[1], 10);
		}

		// Avatars
		const myAvatar = body.match(
			new RegExp(
				`<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="${this.manager.steamID.accountid}">`,
			),
		);
		const theirAvatar = body.match(
			new RegExp(
				`<img src="([^"]+)"( alt="[^"]*")? data-miniprofile="${this.partner.accountid}">`,
			),
		);
		if (myAvatar) {
