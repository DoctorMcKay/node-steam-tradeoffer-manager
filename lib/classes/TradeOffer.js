...
TradeOffer.prototype.addMyItemsByName = function(items) {
	        return new Promise((resolve, reject) => {
            // Find the item by its name in the user's inventory
            this.returnedItem.manager.getUserInventoryContents(this.returnedItem.partner, 730, 2, true, (err, items) => {
                if (err) {
                ...
                if (item) {
                    const result = this.returnedItem.addMyItem(item);
                    resolve(result);
                } else {
