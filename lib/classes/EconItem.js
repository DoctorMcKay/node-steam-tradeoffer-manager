...
function EconItem(item) {
	...
	if (this.appid == 753 && !this.market_fee_app && this.market_hash_name) {
		let match = this.market_hash_name.match(/^(\d+)-/);
		if (match) {
...
EconItem.prototype.getLargeImageURL = function() {
	return this.icon_url_large ? `https://steamcommunity-a.akamaihd.net/economy/image/${this.icon_url_large}/` : this.getImageURL();
};
