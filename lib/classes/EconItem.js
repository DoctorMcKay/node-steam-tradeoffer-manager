...
EconItem.prototype.isCard = function() {
	if (item.appid != 753 || item.contextid != 6) {		
		return false; // appid is not "Steam" or contextid is not "Community"
	}

	let item_class = item.getTag('item_class');
	if (!item_class || item_class.internal_name != 'item_class_2') {
		return false; 	// Not a trading card
	}

	return true;
};

EconItem.prototype.isRegularCard = function() {
	if (!this.isCard()) {		
		return false;
	}

	let cardborder = item.getTag('cardborder');
	if (!cardborder || cardborder.internal_name != 'cardborder_0') {
		return false; 	// Not regular border
	}
...
EconItem.prototype.isFoilCard = function() {
	if (!this.isCard()) {		
		return false;
	}

	let cardborder = item.getTag('cardborder');
	if (!cardborder || cardborder.internal_name != 'cardborder_1') {
		return false; 	// Not foil border
	}
