...
TradeSession.prototype._handleTradeStatus = function(status) {
	...
	if (status.trade_status > ETradeSessionStatus.TurnedIntoTradeOffer) {
		...
		this._setEnded();
		this._getTradeOffer(status.tradeid, 1);
		return;
				...
				case ETradeSessionAction.AddItem:
					...
					if (isUs) {
						this.getInventory(event.appid, event.contextid, (err, inv) => {
							...
							} else if (this.itemsToGive.some(tradeItem => Helpers.itemEquals(tradeItem, item))) {
								// this item is already in the trade, do nothing
							} else {
	...
	if (status.me && status.them) {
		...
		if (!this._loadingInventory()) {
			for (let i = 0; i < 2; i++) {
				...
				let localAssets = i == 0 ? this.itemsToGive : this.itemsToReceive;
...
TradeSession.prototype._getTradeOffer = function(offerID, attemptNum) {
	if (attemptNum > 5) {
		this.emit('end', ETradeSessionStatus.TurnedIntoTradeOffer, offerID, null);
		return;
	}

	this._manager.getOffer(offerID, (err, offer) => {
		if (err) {
			this.emit('debug', 'Error getting trade offer ' + offerID + ': ' + err.message);
			setTimeout(() => this._getTradeOffer(offerID, attemptNum + 1), 500);
			return;
		}

		this.emit('end', ETradeSessionStatus.TurnedIntoTradeOffer, offerID, offer);
	});
};
