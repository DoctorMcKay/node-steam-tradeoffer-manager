...
TradeOfferManager.prototype.openTradeSession = function(partner, callback) {
	...
	Helpers.getUserDetailsFromTradeWindow(this, `https://steamcommunity.com/trade/${partner.getSteamID64()}`).then(({me, them}) => {
...
TradeSession.prototype._handleTradeStatus = async function(status) {
	...
	if (status.events) {
		try {
			for (let keyStr in status.events) {
				if (!status.events.hasOwnProperty(keyStr)) {
					continue;
				}
			...
			// This should only happen if the trade is fatally errored
			this.emit('debug', ex.message);
			return;
		}
	}
	...
	if (status.me && status.them) {
		['ready', 'confirmed'].forEach((thingToCheck) => {
			for (let i = 0; i < 2; i++) {
				...
				if (local != remote) {
					if (thingToCheck == 'ready') {
						this._handleEvent({
							"action": remote ? ETradeSessionAction.Ready : ETradeSessionAction.Unready,
							who
						});
					} else if (thingToCheck == 'confirmed' && remote) {
						this._handleEvent({
							"action": ETradeSessionAction.Confirm,
							who
						});
					}

					this.emit('debug', `Trade got out of sync. Local ${thingToCheck} status for ${who} is ${local} but we got ${remote}. Forging appropriate event.`);
				}
			}
		});
		...
		if (!this._loadingInventory()) {
			for (let i = 0; i < 2; i++) {
				...
				let localAssets = (i == 0 ? this.itemsToGive.concat(this._itemsToGiveRemoving) : this.itemsToReceive).filter(item => !item.pendingAdd);

				if (remoteAssets) {
					if (remoteAssets.length != localAssets.length) {
						for (let j = 0; j < remoteAssets.length; j++) {
							if (!localAssets.some(localAsset => Helpers.itemEquals(localAsset, remoteAssets[i]))) {
								// It's a new asset
								await this._handleEvent({
									"action": ETradeSessionAction.AddItem,
									who,
									"appid": remoteAssets[i].appid,
									"contextid": remoteAssets[i].contextid,
									"assetid": remoteAssets[i].assetid || remoteAssets[i].id
								});

								this.emit('debug', `Trade got out of sync and we spoofed an AddItem event for ${who} for item ${remoteAssets[i].appid}_${remoteAssets[i].contextid}_${remoteAssets[i].assetid}`);
							}
						}

						for (let j = 0; j < localAssets.length; j++) {
							if (!remoteAssets.some(remoteAsset => Helpers.itemEquals(remoteAsset, localAssets[i]))) {
								// It's a removed asset
								await this._handleEvent({
									"action": ETradeSessionAction.RemoveItem,
									who,
									"appid": localAssets[i].appid,
									"contextid": localAssets[i].contextid,
									"assetid": localAssets[i].assetid || localAssets[i].id
								});

								this.emit('debug', `Trade got out of sync and we spoofed a RemoveItem event for ${who} for item ${remoteAssets[i].appid}_${remoteAssets[i].contextid}_${remoteAssets[i].assetid}`);
							}
						}
					}
...
TradeSession.prototype._handleEvent = function(event) {
	return new Promise((resolve, reject) => {
		if (!event.steamid && !event.who) {
			return reject(this._terminateWithError("Unknown who committed event " + event.action + "!"));
		}

		let isUs = event.steamid == this._manager.steamID.getSteamID64() || event.who == 'me';
		this.emit('debug', 'Handling event ' + (ETradeSessionAction[event.action] || event.action) + ' - ' + JSON.stringify(event));
		...
		switch (parseInt(event.action, 10)) {
			case ETradeSessionAction.AddItem:
				...
				if (isUs) {
					this.getInventory(event.appid, event.contextid, (err, inv) => {
						if (err) {
							return reject(this._terminateWithError("Cannot get my inventory: " + err.message));
						}

						let item = inv.filter(item => item.assetid == event.assetid)[0];
						let filtered;
						if (!item) {
							return reject(this._terminateWithError(`Could not find item ${event.appid}_${event.contextid}_${event.assetid} in our inventory even though it was added to the trade`));
						}

						if ((filtered = this.itemsToGive.filter(tradeItem => Helpers.itemEquals(tradeItem, item))).length > 0) {
					...
					this._getTheirInventory(event.appid, event.contextid, (err, inv) => {
						if (err) {
							return reject(this._terminateWithError("Cannot get partner inventory: " + err.message));
						}

						let item = inv.filter(item => item.assetid == event.assetid)[0];
						if (!item) {
							return reject(this._terminateWithError(`Could not find item ${event.appid}_${event.contextid}_${event.assetid} in partner's inventory even though it was added to the trade`));
						}

						if (this.itemsToReceive.some(tradeItem => Helpers.itemEquals(tradeItem, item))) {
			...
			case ETradeSessionAction.Confirm:
				...
				} else if (!isUs && !this.them.confirmed) {
