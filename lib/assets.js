...
TradeOfferManager.prototype._mapItemsToDescriptions = function(appid, contextid, items) {
	...
	return items.map(function(item) {
		...
		for(i in cache[key]) {
			if(['market_tradable_restriction', 'market_marketable_restriction'].indexOf(i) != -1) {
				item[i] = parseInt(cache[key][i], 10);
			} else if(['tradable', 'marketable', 'commodity'].indexOf(i) != -1) {
				item[i] = !!parseInt(cache[key][i], 10);
			} else {
				item[i] = cache[key][i];
			}
