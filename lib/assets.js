...
TradeOfferManager.prototype._digestDescriptions = function (descriptions) {
	...
	if (descriptions && !(descriptions instanceof Array)) {
		descriptions = Object.keys(descriptions).map(
	...
	(descriptions || []).forEach((item) => {
		...
		this._persistToDisk(
			`asset_${item.appid}_${item.classid}_${
				item.instanceid || "0"
			}.json`,
			JSON.stringify(item)
		);
...
TradeOfferManager.prototype._mapItemsToDescriptions = function (
	...
	return items.map((item) => {
		...
		var instanceid = item.instanceid || "0";
		var key = `${item.appid}_${item.classid}_${instanceid}`;
		...
		const uniqueFields = ["assetid", "contextid", "instanceid"];
		for (let i in entry) {
			if (entry.hasOwnProperty(i) && !uniqueFields.includes(i)) {
				item[i] = entry[i];
			}
		}
...
TradeOfferManager.prototype._addDescriptions = function (items, callback) {
	var descriptionRequired = items.filter(
...
TradeOfferManager.prototype._requestDescriptions = function (
	...
	var getFromSteam = () => {
		...
		classes.forEach((item) => {
			...
			if (
				apps[index].indexOf(
					item.classid + "_" + (item.instanceid || "0")
				) == -1
			) {
					...
					(chunk, chunkCb) => {
						...
						this._apiCall(
							...
							(err, body) => {
								...
								var chunkItems = Object.keys(body.result)
									.map((id) => {
										if (!id.match(/^\d+(_\d+)?$/)) {
											return null;
										}
