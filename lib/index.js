...
TradeOfferManager.prototype.setCookies = (cookies, callback) => {
	if (this.hasShutDown) {
...
TradeOfferManager.prototype.shutdown = () => {
...
TradeOfferManager.prototype.parentalUnlock = (pin, callback) => {
...
TradeOfferManager.prototype._checkApiKey = (callback) => {
	if (this.apiKey) {
		if (callback) {
			callback();
		}
...
TradeOfferManager.prototype.loadInventory = (appid, contextid, tradableOnly, callback) => {
	this.loadUserInventory(this.steamID, appid, contextid, tradableOnly, callback);
};

TradeOfferManager.prototype.loadUserInventory = (uid, appid, contextid, tradableOnly, callback) => {
	this._community.getUserInventory(uid, appid, contextid, tradableOnly, callback);
};

TradeOfferManager.prototype.getOfferToken = (callback) => {
...
TradeOfferManager.prototype.getOffersContainingItems = (items, includeInactive, callback) => {
