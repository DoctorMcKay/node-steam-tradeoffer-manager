...
require("node:util").inherits(TradeOfferManager, require("node:events").EventEmitter);
...
function TradeOfferManager(options) {
	...
	const assetCacheGcInterval = options.assetCacheGcInterval || 120000; // 2 minutes
	...
	if (!options.dataDirectory && options.dataDirectory !== null) {
		if (process.env.OPENSHIFT_DATA_DIR) {
			options.dataDirectory =
				`${process.env.OPENSHIFT_DATA_DIR}/node-steam-tradeoffer-manager`;
		} else {
...
TradeOfferManager.prototype.setCookies = function (
	...
	try {
		...
		const cookieValueMatch = loginSecureCookie.match(/steamLoginSecure=([^;]+)/);
		if (!cookieValueMatch) {
	...
	if (this._getPollDataFromDisk) {
		this._getPollDataFromDisk = undefined;
...
TradeOfferManager.prototype.createOffer = function (partner, token) {
	if (typeof partner === "string" && partner.match(/^https?:\/\//)) {
		...
		const url = require("node:url").parse(partner, true);
...
TradeOfferManager.prototype._warn = (msg) => {
