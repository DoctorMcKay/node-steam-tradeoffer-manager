...
TradeOfferManager.prototype.setCookies = function(cookies, familyViewPin, callback) {
	...
	return StdLib.Promises.callbackPromise([], callback, true, async (resolve, reject) => {
		...
		if (this._getPollDataFromDisk) {
			...
			let files = await this._getFromDisk([filename]);
			if (files[filename]) {
				try {
		...
		if (familyViewPin) {
			await this.parentalUnlock(familyViewPin);
		}
 ...
 * @private
 */
TradeOfferManager.prototype._checkApiKey = function() {
...
TradeOfferManager.prototype.getInventoryContents = function(appid, contextid, tradableOnly, callback) {
	return StdLib.Promises.callbackPromise(['inventory', 'currencies'], callback, false, async (resolve, reject) => {
		// are we logged in?
		if (!this.steamID) {
			return reject(new Error('Not Logged In'));
		}

		resolve(await this.getUserInventoryContents(this.steamID, appid, contextid, tradableOnly));
	});
};
...
TradeOfferManager.prototype.getUserInventoryContents = function(sid, appid, contextid, tradableOnly, callback) {
	return StdLib.Promises.callbackPromise(['inventory', 'currencies'], callback, false, (resolve, reject) => {
...
TradeOfferManager.prototype.getOfferToken = function(callback) {
	return StdLib.Promises.callbackPromise(['token'], callback, false, (resolve, reject) => {
...
TradeOfferManager.prototype.getOffersContainingItems = function(items, includeInactive, callback) {
	...
	return StdLib.Promises.callbackPromise(['sent', 'received'], callback, false, (resolve, reject) => {
		...
		this.getOffers(includeInactive ? EOfferFilter.All : EOfferFilter.ActiveOnly, (err, sent, received) => {
			err ? reject(err) : resolve({sent: sent.filter(filterFunc), received: received.filter(filterFunc)});
		});
