...
function TradeOfferManager(options) {
	...
	if (this._language) {
		var lang = languages.getLanguageInfo(this._language);
		if (!lang.name) {
			this._language = null;
	...
	if (this._steam) {
		...
		// This is an instance of https://www.npmjs.com/package/steam-user,
		// and newItems is emitted when new items are announced.
...
TradeOfferManager.prototype.createOffer = function (partner, token) {
	if (typeof partner === 'string' && partner.match(/^https?:\/\//)) {
		...
		var tokenUrl = url.parse(partner, true);
		if (!tokenUrl.query.partner) {
			throw new Error('Invalid trade URL');
		}

		partner = SteamID.fromIndividualAccountID(tokenUrl.query.partner);
		token = tokenUrl.query.token;
	}
...
TradeOfferManager.prototype.getOffer = function (id, callback) {
	this._apiCall('GET', 'GetTradeOffer', 1, { tradeofferid: id }, (getOfferErr, body) => {
		if (getOfferErr) {
			callback(getOfferErr);
			return;
		}
		...
		Helpers.checkNeededDescriptions(this, [body.response.offer], (descriptionsErr) => {
			if (descriptionsErr) {
				callback(descriptionsErr);
				return;
			}
...
TradeOfferManager.prototype.getOffers = function (filter, historicalCutoff, callback) {
	...
	// Currently the GetTradeOffers API doesn't include app_data,
	// so we need to get descriptions from the WebAPI.
	...
	this._apiCall('GET', 'GetTradeOffers', 1, options, (getTradeOffersErr, body) => {
		if (getTradeOffersErr) {
			callback(getTradeOffersErr);
			return;
		}
		...
		// Make sure at least some offers are well-formed.
		// Apparently some offers can be empty just forever. Because Steam.
		var allOffers = (body.response.trade_offers_sent || [])
		...
		// them for future use.
		var offers = (body.response.trade_offers_sent || [])
			.concat(body.response.trade_offers_received || []);

		Helpers.checkNeededDescriptions(this, offers, (descriptionsErr) => {
			if (descriptionsErr) {
				callback(new Error('Descriptions: ' + descriptionsErr.message));
				return;
			}

			var sent = (body.response.trade_offers_sent || [])
				.map(data => Helpers.createOfferFromData(this, data));
			var received = (body.response.trade_offers_received || [])
				.map(data => Helpers.createOfferFromData(this, data));
