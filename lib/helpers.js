...
Helpers.checkNeededDescriptions = function(manager, offers, callback) {
...
Helpers.getUserDetailsFromTradeWindow = function(manager, url, callback) {
	manager._community.httpRequestGet(url, (err, response, body) => {
		...
		// Escrow
		let myEscrow = body.match(/let g_daysMyEscrow = (\d+);/);
		let theirEscrow = body.match(/let g_daysTheirEscrow = (\d+);/);
