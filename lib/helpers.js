...
exports.itemEquals = (a, b) => (
		a.appid === b.appid &&
		a.contextid === b.contextid &&
		(a.assetid || a.id) === (b.assetid || b.id)
	);

exports.makeAnError = (error, callback, body) => {
	if (callback) {
		if (body?.strError) {
			...
			if (match) {
				error.eresult = Number.parseInt(match[1], 10);
			}
...
exports.checkNeededDescriptions = (manager, offers, callback) => {
...
exports.createOfferFromData = (manager, data) => {
	...
	offer.confirmationMethod =
		data.confirmation_method || EConfirmationMethod.None;
	offer.escrowEnds = data.escrow_end_date
